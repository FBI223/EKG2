...
def get_record_ids(mitdb_path):
    return sorted(list(set(f.split('.')[0] for f in os.listdir(mitdb_path) if f.endswith('.hea'))))

def load_ecg_data(mitdb_path, record_ids, target_fs=360, label_map=None):
...
def preprocess_signals(signals, labels, optimal_segment_length, label_map, unknown_signals):
...
def main():
    ...
    signals, labels, rr_intervals, unknown_signals = load_ecg_data(mitdb_path, record_ids, label_map=label_map)
    optimal_segment_length = determine_optimal_segment_length(rr_intervals)

    X, y = preprocess_signals(signals, labels, optimal_segment_length, label_map, unknown_signals)
    ...
    model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=32, batch_size=32, callbacks=[tf.keras.callbacks.EarlyStopping(patience=4, restore_best_weights=True)])
    ...
    model.save("ecg_classifier_mitbih_unknown.h5")
